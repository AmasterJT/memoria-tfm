...
MITRE ATT\&CK (\textit{Adversarial Tactics, Techniques and Common Knowledge}) es una base de conocimiento desarrollada por MITRE Corporation que documenta tácticas y técnicas empleadas por atacantes reales durante operaciones ofensivas sobre sistemas informáticos\numcite{mitre_attack}.
...
La exposición de credenciales es una causa habitual de compromiso en entornos corporativos. Puede producirse mediante archivos de configuración inseguros, copias de seguridad accesibles, credenciales embebidas en código fuente, variables de entorno expuestas o contraseñas almacenadas sin protección adecuada.

OWASP recomienda proteger adecuadamente secretos como credenciales, claves o tokens reutilizables\numcite{owasp_secrets_management}.

Una vez obtenidas, estas credenciales pueden permitir al atacante acceder a nuevos sistemas y ampliar el alcance de la intrusión. En el laboratorio, esta debilidad conecta la explotación web inicial con fases posteriores de escalada, pivoting y movimiento lateral.
...
\textit{PATH Hijacking} es una técnica de escalada de privilegios que aprovecha configuraciones inseguras de la variable de entorno \texttt{PATH}, utilizada por el sistema para localizar ejecutables cuando no se indica su ruta absoluta.

El riesgo aparece cuando un script ejecutado con privilegios elevados invoca comandos sin ruta completa. En ese caso, un atacante puede colocar un ejecutable malicioso con el mismo nombre en un directorio controlado y conseguir que sea ejecutado antes que el binario legítimo. La documentación de GNU Bash describe el funcionamiento de variables como \texttt{PATH} dentro del intérprete\numcite{gnu_bash_manual}.

En el laboratorio, esta técnica permite mostrar cómo una mala práctica en un script administrativo puede convertir un acceso limitado en un compromiso completo del servidor Linux.
...
\begin{figure}[H]
    ...
    \vspace{0.2cm}
    \captionazul{Ejemplo conceptual de una operación de \textit{pivoting} sobre una red segmentada.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/Active-Directory-logo.png}
