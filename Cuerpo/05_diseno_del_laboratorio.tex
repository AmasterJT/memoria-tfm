...
Las capturas mostradas en la Figura~\ref{fig:optimizacion_memoria_lab} reflejan el consumo real de memoria de las tres máquinas virtuales tras completar el proceso de optimización. Los resultados obtenidos muestran que Windows Server 2012, Windows 7 y Ubuntu Server disponen de memoria suficiente para ejecutar los servicios necesarios del laboratorio.

En el caso del controlador de dominio DC01, con únicamente 1.2 GB de memoria asignada, el sistema mantiene aproximadamente 400 MB disponibles una vez iniciados Active Directory y el servicio DNS. De forma similar, la estación de trabajo DEV01 conserva varios cientos de megabytes libres con una asignación total de 1 GB, mientras que el servidor Ubuntu mantiene memoria suficiente tras el despliegue de Docker y del portal vulnerable.

La optimización realizada no se limitó a la memoria RAM. También se redujo la capacidad máxima asignada a los discos virtuales, ajustándola al uso real observado durante las pruebas. De este modo, se validó una configuración de 20 GB para el servidor Ubuntu, 20 GB para DC01 y 16.5 GB para DEV01, manteniendo margen suficiente para el sistema operativo, los servicios desplegados y las evidencias generadas durante los ejercicios.
...
La Figura~\ref{fig:tamanio_lab_disco_real} muestra el espacio ocupado por las tres máquinas virtuales que forman la infraestructura vulnerable una vez finalizado su despliegue. Durante las pruebas realizadas, el conjunto formado por el servidor web Ubuntu, el controlador de dominio DC01 y la estación de trabajo DEV01 ocupaba aproximadamente 35.7 GB en el equipo anfitrión.

Este tamaño es considerablemente inferior a la capacidad máxima configurada para los discos virtuales, debido al empleo de discos de crecimiento dinámico, que únicamente consumen el espacio realmente utilizado por cada sistema.

En consecuencia, los recursos inicialmente recomendados por VMware Workstation pudieron reducirse de forma significativa sin afectar al comportamiento del laboratorio ni a la reproducibilidad de las cadenas de ataque implementadas. Gracias a este proceso de optimización, la infraestructura completa puede ejecutarse utilizando aproximadamente 3.4 GB de memoria RAM y ocupando menos de 36 GB de almacenamiento real.

Los valores finalmente adoptados fueron empleados durante todas las fases de validación y explotación descritas en el Capítulo~\ref{sec:explotacion_del_laboratorio}, verificándose que no introducen limitaciones apreciables en el desarrollo de los distintos escenarios ofensivos. De esta forma, el diseño final mantiene un equilibrio entre realismo, accesibilidad y eficiencia en el consumo de recursos, facilitando su despliegue en equipos domésticos o académicos con capacidades moderadas.
